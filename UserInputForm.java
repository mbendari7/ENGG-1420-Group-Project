...
public class UserInputForm {
    public static void main(String[] args) {
        ...
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // tells java when X button is closed, program stops
        frame.setSize(350, 250); // we can modify these later (width & height of window in pxls)

        // GridLayout just organizes the items in a 5x2 grid. the 10, 10 adds gaps between boxes (we can modify)

        JPanel panel = new JPanel(new GridLayout(5, 2, 10, 10));

        // Jlabel is text that user CANNOT change... JTextField is the empty white box where user types their info.
        ...
        panel.add(new JComboBox<>(types)); // JComboBox is simply just a dropdown menu,.. takes the list and lets user click from them.
